...
export class ContentComponent implements OnInit {
  ...
  async onSubmit() {
    const validation = await this.connectionService.getData('specialties', 'nome', this.form.value.nome.toLowerCase().toLocaleUpperCase());
    if (validation) {
      this.toast.remove;
      ...
      if (!this.isFormUpdate) {
       try {
        const insert = await this.connectionService.createData('specialties', data);

        if (insert) {
          this.modalService.dismissAll();
          this.toast.remove;
          this.toast.success('Tipo de estabelecimento adicionado...', 'Parabéns!', {
            positionClass: 'toast-top-right',
            progressBar: true
          });
        }
        ...
        try {
          const update = await this.connectionService.updateData('specialties', this.form.value.id, data);

          if (update) {
            this.modalService.dismissAll();
            this.toast.remove;
            this.toast.success('Tipo de estabelecimento atualizado...', 'Parabéns!', {
              positionClass: 'toast-top-right',
              progressBar: true
            });
          }
  ...
  ngOnInit(): void {
    ...
    const sub = this.connectionService.getAllDatas('specialties').subscribe((data) => {
      this.itens = data;
    })
  ...
  async remove(item: Especialidade) {
    const sweet = await this.connectionService.confirmDelete('Atenção!', 'Deseja remover este registro?');

    if (sweet) {
      try {
        const confirmDelete = await this.connectionService.deleteData('specialties', item.id);
        debugger;
        if (confirmDelete) {
          this.toast.remove;
          this.toast.success('Tipo de estabelecimento excluído...', 'Parabéns!', {
            positionClass: 'toast-top-right',
            progressBar: true
          });
        }
